(function() {
   ...
   * Taken from https://github.com/twitter/twitter-text-js
   *
   * @param {String} text The text to escape (must not be already escaped or contain HTML tags)
   *
   * @return {String} The escaped text
   *
   * @api public
   */
  txt.htmlEscape = function(text) {
    return text && text.replace(/[&><"']/g, function(c) { return HTML_ENTITIES[c]; });
  };
  var HTML_ENTITIES = {
    '&': '&amp;',
    '>': '&gt;',
   ...
   *
   * @return {String} The prettified text
   *
   * @api public
   */
   ...
   *
   * @param {String} s The string with BOM
   *
   * @return {String} The string without BOM
   *
   * @api public
   */
  txt.removeByteOrderMark = function(s) {
  ...
  var UNICODE_SPACES = [
    ...
    String.fromCharCode(0x0009), // White_Space # Cc [5] <control-0009>..<control-000D>
    String.fromCharCode(0x000A),
    String.fromCharCode(0x000B),
    String.fromCharCode(0x000C),
    String.fromCharCode(0x000D),
    String.fromCharCode(0x2000), // White_Space # Zs [11] EN QUAD..HAIR SPACE
  ...
  /**
   * Transliterate the given string to ASCII.
   * The resulting representation tries to convey in Roman letters the pronunciation expressed by the text in any writing system.
   * See also the Node module, node-unidecode.
   *
   * @param {String} s The string in UTF-8
   *
   * @return {String} The transliterated string, guaranteed to be in the ASCII range
   *
   * @api public
   */
  txt.transliterate = function transliterate(s) {
    if (!s) return '';
    var r = '';
    s = s.replace(txt.spaces, ' ');
    for (var i = 0; i < s.length; i++) {
      r += ACCENT_MAP[s.charAt(i)] || s.charAt(i);
    }
    return r;
  };
