...
Mognet runs as an in-memory, single-threaded server designed to run on cheap hosting services and still capable of withstanding a sudden popularity. It prefers to generate pages once and then serve them from memory, but it can also build pages dynamically on each request. Once it has loaded a page for the first time it doesn't hit the disk ever again, because hard disks are really slow. If you have a lot of traffic keep in mind that Mognet can't do anything to save your bandwidth (except caching and compressing HTTP responses), so if you publish a lot of images and other big static files it's probably a good idea to host them in the Cloud, and link to them from your pages.
...
- A template for the web pages done in HTML5 & CSS3. It features a fluid & responsive layout, an obsessive care for typographic details and absolutely no image files. This design is currently used at [the Author's personal site]().
...
- ``HUP (1)``: Updates the index file and reloads it in the server, clearing the content cache. Also reloads the configuration. Use this when you update the site contents, HTML templates or configuration (not needed when you update static files);
...
If you have redirected the server standard output to a file (``/var/www/log/stdout.log`` by default), of course to see what Mognet says you will have to open that file.



## Configuration

Mognet runs by default on the standard port 80, without writing any log messages (except at startup). You can change the default configuration in two ways:
...
- ``pageSize``: (default: 12) Number of articles displayed on each index page.
